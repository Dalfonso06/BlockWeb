...
## Icons
- All icons come from `react-icons/fa6` (Font Awesome 6), routed through the central registry at `src/icons/index.tsx` — never import from `react-icons/fa6` (or any other icon source) directly in components, pages, or features.
...
- If nothing existing fits, import the appropriate icon from `react-icons/fa6`, add a new semantically-named export for it in `src/icons/index.tsx` (named for what it means in the UI, not the underlying glyph name), then import and use that new export at the call site.
